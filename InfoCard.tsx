...
export const InfoCard: React.FC<InfoCardProps> = ({ item, onExpand }) => {
  ...
  return (
    ...
    `}>
      ...
      <div className={`px-6 py-4 border-t flex items-center justify-between gap-2 ${isHostage ? 'bg-orange-50/50 border-orange-100' : 'bg-gray-50 border-gray-100'}`}>
        <button 
          onClick={() => onExpand(item)}
          className={`text-sm font-semibold flex items-center gap-1 transition-colors ${isHostage ? 'text-amber-700 hover:text-amber-900' : 'text-gray-600 hover:text-[#005A8D]'}`}
        >
          הרחבה בנושא
        ...
        {item.link && (
          <a 
            ...
            className={`font-bold text-sm hover:underline flex items-center group ${isHostage ? 'text-amber-700' : 'text-[#005A8D]'}`}
          >
            <span>למידע המלא</span>
